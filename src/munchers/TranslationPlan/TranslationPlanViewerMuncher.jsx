      ...
      const getVerseText = async () => {
        if (selectedBurrito) {
          ...
          if (usfmResponse.ok) {
            ...
            const sbSelectedScriptDirSet =
              selectedBurritoSbTextDir === "ltr" ||
              selectedBurritoSbTextDir === "rtl";
            ...
            const newVerseText = Object.fromEntries(
              result.data.documents[0].cvIndexes.map((i) => [
                i.chapter,
                Object.fromEntries(
                  i.verses
                    .map((v, n) => [
                      `${n}`,
                      v.verse.length > 0 ? v.verse[0].text : [],
                    ])
  ...
  const isInInterval = (section, systemBcv) => {
    ...
    return (
      (chapterNum > startChapter ||
        (chapterNum === startChapter && verseNum >= startVerse)) &&
            ...
            filteredStories.map((c, i) => (
              <Tooltip key={i} title={c.fieldInitialValues.reference}>
                <MenuItem
                  ...
                  selected={
                    selectedStory === c.fieldInitialValues.sectionNumber
                  }
                >
                  {c.fieldInitialValues.sectionNumber} -{" "}
                  {c.fieldInitialValues.sectionTitle}
                </MenuItem>
              </Tooltip>
            ))
          )}
            ...
            {section ? (
              <Box sx={{ padding: 1 }}>
                ...
                {planIngredient.sectionStructure.map((field, i) => {
                  ...
                  if (
                    Object.keys(verseText).length > 0 &&
                    field.type === "scripture"
                  ) {
                    let chapterN = "0";
                    return (
                        ...
                        {section.paragraphs.map((p) => {
                          if (p.units) {
                            ...
                            return (
                              <>
                                {newChapter && (
                                  <div className="marks_chapter_label">{c}</div>
                                )}
                                <div className={p.paraTag}>
                                  {p.units.map((cv) => (
                                    <span>
                                      <span className="marks_verses_label">
                                        {cv.split(":")[1]}{" "}
                                      </span>
                                      <span>
                                        {
                                          verseText[cv.split(":")[0]][
                                            cv.split(":")[1]
                                          ]
                                        }{" "}
                                      </span>
