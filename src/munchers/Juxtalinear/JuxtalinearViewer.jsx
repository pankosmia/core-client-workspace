...
function JuxtalinearViewerMuncher({ metadata }) {
    ...
    const sentencesForVerse = ingredient.filter(i => i.chunks.some(c => c.source.some(s => s.cv === `${systemBcv.chapterNum}:${systemBcv.verseNum}`)))
    ...
    const sentencesForVerses = ingredient.filter(i => i.chunks.some(c => c.source.some(s => uniqueVersesInSentence.includes(s.cv))))
                        ...
                        <Grid2 container>
                            {
                                sentencesForVerses
                                    .map(
                                        s => <Grid2 container size={12} spacing={0} sx={{ marginBottom: 1 }} display="flex">
                                            {
                                                s.chunks
                                                    .map(
                                                        c => <>
                                                            <Grid2 item size={6} spacing={0} sx={{ p: 0, m: 0 }} justifyContent="flex-end">
                                                                <Box display="flex" justifyContent="flex-end" alignContent="flex-end" alignItems="flex-end" sx={{ fontSize: "small", pr: 1, textAlign: "right" }}>
                                                                    {c.source.map(s => s.content).join(" ")}
                                                                </Box>
                                                            </Grid2>
                                                            <Grid2 item size={6} sx={{ fontSize: "small", pl: 1, m: 0, }} spacing={0}>
                                                                {c.gloss}
                                                            </Grid2>
                                                        </>
                                                    )
